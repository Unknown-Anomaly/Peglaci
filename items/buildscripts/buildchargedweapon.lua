...
function build(directory, config, parameters, level, seed)
  ...
  if config.tooltipKind ~= "base" then
    ...
    for i, chargeLevel in ipairs(config.primaryAbility.chargeLevels) do
      config.tooltipFields.chargeLevelsLabel = config.tooltipFields.chargeLevelsLabel .. sb.print(config.primaryAbility.chargeLevels[i].time) .. ", "
    end
    
    config.tooltipFields.chargeLevelsLabel = string.sub(config.tooltipFields.chargeLevelsLabel, 0, string.len(config.tooltipFields.chargeLevelsLabel) - 2)
